...
def _fill_confidence_window(
    ...
    result = None
    for _idx in range(5):
        result = actuator.apply(MAC, measured_latency_ms, target_total_ms, current_filter_delay)
...
def test_subsequent_burst_within_threshold_does_not_correct() -> None:
    ...
    result = _fill_confidence_window(actuator, 370.75, 370.0, 20.0)
...
def test_subsequent_burst_above_threshold_applies_set_delay_with_correct_sign() -> None:
    ...
    result = _fill_confidence_window(actuator, 407.0, 370.0, 50.0)

    assert result.action == "corrected"
    assert result.delta_ms == 37.0
...
def test_apply_with_negative_offset_increases_delay() -> None:
    ...
    result = _fill_confidence_window(actuator, 333.0, 370.0, 20.0)

    assert result.action == "corrected"
    assert result.delta_ms == -37.0
    assert result.clock_prior_reset is True
    assert calls == [("/tmp/AA_BB_CC_DD_EE_FF.sock", "set_delay 57.000")]
...
def test_burst_amp_escalates_on_each_miss_and_deescalates_on_success() -> None:
    calls: list[tuple[str, str]] = []
    actuator = _actuator(calls)
...
def test_slider_aware_clock_prior_reset_cycles_still_emitted() -> None:
    ...
    result = _fill_confidence_window(actuator, 335.0, 370.0, 50.0)

    assert result.action == "corrected"
    assert result.delta_ms == -35.0
    assert result.clock_prior_reset is True
    assert calls == [("/tmp/AA_BB_CC_DD_EE_FF.sock", "set_delay 85.000")]
...
def test_startup_tune_convergence_with_fixed_target() -> None:
    ...
    # A speaker measuring 540ms against a 500ms target with current_filter_delay 100ms
    # should be told to subtract 40ms of filter delay so its NEXT cycle measures closer
    # to 500ms.
    calls: list[tuple[str, str]] = []
    actuator = _actuator(calls)
    _establish_baseline(actuator)

    result = _fill_confidence_window(actuator, 540.0, 500.0, 100.0)
    assert result.action == "corrected"
    assert result.delta_ms == 40.0
    assert calls == [("/tmp/AA_BB_CC_DD_EE_FF.sock", "set_delay 60.000")]
    ...
    result2 = _fill_confidence_window(actuator2, 460.0, 500.0, 100.0)
    assert result2.action == "corrected"
    assert result2.delta_ms == -40.0
    assert calls == [("/tmp/AA_BB_CC_DD_EE_FF.sock", "set_delay 140.000")]
...
def test_corrected_action_appends_runtime_correction_jsonl(tmp_path: Path) -> None:
    ...
    assert event["measured_latency_ms"] == 407.0
    assert event["target_total_ms"] == 370.0
    assert event["current_filter_delay_ms"] == 50.0
    assert event["delta_ms"] == 37.0
