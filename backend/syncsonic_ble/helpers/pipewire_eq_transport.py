...
DEFAULT_XDG_RUNTIME_DIR = "/run/syncsonic"
DEFAULT_PULSE_SERVER = "unix:/run/syncsonic/pulse/native"


def _pipewire_subprocess_env() -> dict[str, str]:
    env = os.environ.copy()
    runtime_dir = env.get("RUNTIME_DIRECTORY") or DEFAULT_XDG_RUNTIME_DIR
    env["XDG_RUNTIME_DIR"] = runtime_dir
    env["PULSE_SERVER"] = DEFAULT_PULSE_SERVER
    env["PULSE_SYSTEM_BUS"] = "1"
...
def _delay_node_name(mac: str) -> str:
...
class PipeWireEqTransportManager:
    ...
    def ensure_eq(self, mac: str, *, enabled: bool = False) -> bool:
        ...
        if not self._process_alive(proc):
            ...
            log.warning("PipeWire EQ filter exited early for %s: %s", mac, err)
            return False
        if not self._stereo_ports_ready(node):
            self._terminate_process(proc)
            return False
        delay_node = _delay_node_name(mac)
        if not self._connect_eq_route(mac, sink_name, node, delay_node):
            self._disconnect_eq_route(sink_name, node, delay_node)
            self._terminate_process(proc)
            return False
    ...
    def _stereo_ports_ready(self, node_name: str, timeout_sec: float = 2.0) -> bool:
        ...
        while time.monotonic() < deadline:
            result = subprocess.run(["pw-cli", "ls", "Port"], capture_output=True, text=True, env=_pipewire_subprocess_env())
            if result.returncode == 0:
                text = result.stdout
                needed = (
    ...
    def _connect_eq_route(
        ...
        # Insert EQ after the existing delay filter so ultrasonic alignment
        # stays intact:
        #   virtual_out -> <delay> -> <eq> -> <speaker sink>
        self._disconnect_route_segment(
            delay_node,
            sink_name,
            [
                (f"{delay_node}:output_FL", f"{sink_name}:playback_FL"),
                (f"{delay_node}:output_FR", f"{sink_name}:playback_FR"),
            ],
        )
        link_pairs = [
            (f"{delay_node}:output_FL", f"{node}:input_FL"),
            (f"{delay_node}:output_FR", f"{node}:input_FR"),
    ...
    def _disconnect_route_segment(
        self,
        delay_node: str,
        sink_name: str,
        pairs: list[tuple[str, str]],
    ) -> None:
        for source_port, target_port in pairs:
            subprocess.run(
    ...
    def _disconnect_eq_route(self, sink_name: str, node: str, delay_node: str) -> None:
        ...
        self._restore_delay_route(sink_name, delay_node)

    def _restore_delay_route(self, sink_name: str, delay_node: str) -> None:
        for source_port, target_port in [
            (f"{delay_node}:output_FL", f"{sink_name}:playback_FL"),
            (f"{delay_node}:output_FR", f"{sink_name}:playback_FR"),
        ]:
    ...
    def _teardown_locked(self, route: Dict[str, Any]) -> None:
        sink = route.get("sink", "")
        node = route.get("node", "")
        delay_node = route.get("delay_node") or _delay_node_name(route.get("mac", ""))
        if sink and node and delay_node:
            self._disconnect_eq_route(sink, node, delay_node)
