...
**Status (2026-05-19).** Slice 0 (open-question experiment) complete. **Ultrasonic at 18.0-19.5 kHz wins on viability** — Pi-validated against the operator's worst-case BT speaker with ~40 dB SNR at the mic, naturally immune to music masking. The existing `analyze_lag` cross-correlation analyzer was found to fail on ultrasonic over BT (A2DP codecs preserve energy but mangle waveform shape); slice 1 needs a new envelope-based detector. Full experimental record + slice-1 architecture in [`proposals/06-ultrasonic-vs-inband.md`](proposals/06-ultrasonic-vs-inband.md); epic-level summary in the charter's "Slice 0 Findings" section. Slice 1 is now well-scoped: cadence-based ultrasonic bursts + envelope detection + the existing elastic-engine `set_rate_ppm` correction path.
