...
**Status (2026-06-03).** **Production-effective on the 2-BT-speaker case.** Slices 0-18 have shipped a complete closed-loop architecture: cross-correlation-on-envelope measurement (slice 15, sub-millisecond precision), single-burst sparse cadence (slice 18, eliminated detector artifacts), symmetric amplitude ladder (slice 18, recovered from sticky-amp audibility regression), and confidence-gated actuation (slice 18.2, median-of-5 window + 2-sigma noise floor stops over-correction from single bad measurements). Verified on Pi 10.0.0.89 via 62-hour soak (6,786 captures) and live operator listening test 2026-06-03. Six areas of future exploration documented in `epics/ultrasonic-runtime-sync.md` "Future exploration" section — most prominently WiFi-speaker PipeWire integration — but **not filed as committed workstreams**. Epic remains unmerged to `main`; promotion is a separate operator-triggered decision.
